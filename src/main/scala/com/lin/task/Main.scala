...
import akka.actor.{Actor, ActorIdentity, ActorPath, ActorSystem, AddressFromURIString, Identify, PoisonPill, Props, RootActorPath}
...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    ...
    if (args.isEmpty) {
      ...
      Thread.sleep(5000)
      startFrontend(0,classOf[DefaultWorkProducer],classOf[WorkResultConsumer],workSource,DefaultResultHandler())
    } else {
      val port = args(0).toInt
      if (2000 <= port && port <= 2999)
        startBackend(port, "backend")
      else if (3000 <= port && port <= 3999)
        startFrontend(port,classOf[DefaultWorkProducer],classOf[WorkResultConsumer],workSource,DefaultResultHandler())
      else
  ...
  def startFrontend[T <: WorkProducer,S <: WorkResultConsumer](port: Int,producer:Class[T],consumer: Class[S],workSource: WorkSource,resultHandler:ResultHandler): Unit = {
    val conf = ConfigFactory.parseString("akka.remote.netty.tcp.port=" + port).
      withFallback(ConfigFactory.load())
    val system = ActorSystem("ClusterSystem", conf)
    val frontend = system.actorOf(Props[Frontend], "frontend")
    system.actorOf(Props(producer, workSource,frontend), "producer")
